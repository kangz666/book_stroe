...
public class RefundJDialog extends JFrame {
...
//			public void run() {
//				try {
//					RefundJDialog frame = new RefundJDialog();
...
//	}
	 ...
	 * Create the frame.
	 */
	public RefundJDialog(Manager m) {
		setTitle("退款订单");
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String string = JOptionPane.showInputDialog(null,"确定退款金额："+o.getPrice()+"？","确定");
				if(string != null) {
					if((string.trim().equals("确定"))) {
					
					boolean doUpdateorder = od.doUpdateorder(o, m, "已退款");
					if (doUpdateorder) {
						...
						rd.inputrecord(m, new Record(m.getManager_id(),o.getOrder_id(),"已退款",stringdate,String.valueOf(o.getPrice())));
						} else {
		...
		btnNewButton.setIcon(new ImageIcon(RefundJDialog.class.getResource("/pictrue/\u6210\u529F.png")));
		panel_1.add(btnNewButton);
		
		JButton btnNewButton_1 = new JButton("拒绝退款");
		btnNewButton_1.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				Connection conn = dc.getConnection();
				OrderDao od = new OrderDaoImpl(conn);
				String string = JOptionPane.showInputDialog(null,"确定拒绝退款？","确定");
				if(string != null) {
					if((string.trim().equals("确定"))) {
					boolean doUpdateorder = od.doUpdateorder(o, m, "拒绝退款");
					if (doUpdateorder) {
						JOptionPane.showMessageDialog(null, "已拒绝退款！");
						init();
						printable(ALLlist);
						//记录
						RecordDao rd =new RecordDaoImpl();
						...
						SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
						String stringdate = sdf.format(date);
						rd.inputrecord(m, new Record(m.getManager_id(),o.getOrder_id(),"拒绝退款",stringdate,String.valueOf(o.getPrice())));
						} else {
							JOptionPane.showMessageDialog(null, "拒绝退款失败！");
						}
				}
			}
				dc.close();
		...
		btnNewButton_1.setIcon(new ImageIcon(RefundJDialog.class.getResource("/pictrue/close_filled.png")));
	...
	private void init() {
		...
		OrderDao od = new OrderDaoImpl(conn);
		this.ALLlist = od.domanagerrefundselect();
